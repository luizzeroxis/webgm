	...
	class ProjectObject {
		...
		class ProjectEvent {
			...
			class ProjectAction {
				...
				// constructor (typelibrary, typeid, appliesTo, relative, not) {
					...
					// Make args with default values
					this.args = [];
					if (['gmfunction'].includes(this.type.kind)) {
						this.type.args.forEach((arg, i) => {
							this.args[i] = arg.default;
						})
					}
				}
				getName() {
