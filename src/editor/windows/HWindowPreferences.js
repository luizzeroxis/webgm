...
export default class HWindowPreferences extends HWindow {
	constructor(manager, editor) {
		...
		parent(this.client);
			...
			this.inputClearCanvasOnStop = add( new HCheckBoxInput("Clear game canvas on stop", this.editor.preferences.get("clearCanvasOnStop")) );
	...
	saveData() {
		this.editor.preferences.setAll({
			...
			clearCanvasOnStop: this.inputClearCanvasOnStop.getChecked(),
